...
import DwarfMoonsScene from "./DwarfMoonsScene";
import type { DwarfMoonsTextures } from "./useDwarfMoonsTextures";

interface DwarfMoonsCanvasProps {
  system: DwarfSystem;
...
export default function DwarfMoonsCanvas(props: DwarfMoonsCanvasProps) {
  return (
    <Canvas
      className="absolute inset-0"
      // Orthographic (no perspective distortion): a true plane-of-sky projection,
      // so a body always appears at exactly its computed (x, y) and z only decides
      // whether it is in front of or behind the central body along the line of sight.
      orthographic
      dpr={[1, 2]}
...
/**
 * Fits the orthographic camera so the framed field (targetReq central-body radii
 * each side of centre, plus a margin) fills the viewport width, and locks it
 * looking straight down the line of sight (-Z), North up. No rotation, so the
 * compass convention stays fixed. Refits on resize and when the zoom preset (or
 * system) changes. The camera sits far back (+Z 600) with a deep far plane so a
 * wide system (Hydra ~55 radii, Hiʻiaka ~63 radii) still fits when framed.
 */
